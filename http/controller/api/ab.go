...
import (
	...
	"github.com/RobertLesgros/rustdesk-interface/v2/http/response/api"
...
func (a *Ab) Ab(c *gin.Context) {
	...
	// Convertir les noms des tags en une chaîne séparée par des virgules
	var tagNames []string
...
// @Accept  json
// @Produce  json
// @Param body body requstform.AddressBookForm true "Formulaire d'adresse"
...
func (a *Ab) Personal(c *gin.Context) {
	...
	*/
	if global.Config.Rustdesk.Personal == 1 {
		...
		// Si guid est retourné, les requêtes suivantes changeront
		c.JSON(http.StatusOK, gin.H{
			"guid": guid,
			"name": user.Username,
			"rule": 3,
...
func (a *Ab) Settings(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{
		"max_peer_one_ab": 0, // Nombre maximum de pairs, 0 signifie illimité
	})
}
...
// @Accept  json
// @Produce  json
// @Param current query int false "Numéro de page"
// @Param pageSize query int false "Quantité par page"
// @Success 200 {object} response.Response
// @Failure 500 {object} response.Response
...
func (a *Ab) SharedProfiles(c *gin.Context) {
	...
	allAbIds := make(map[uint]int) // Utiliser map pour dédupliquer, et conserver la règle maximale
	allUserIds := make(map[uint]*model.User)
	rules := service.AllService.AddressBookService.CollectionReadRules(user)
	for _, rule := range rules {
		// Vérifier d'abord si cela existe
		r, ok := allAbIds[rule.CollectionId]
		if ok {
			// Vérifier ensuite la taille des permissions
			if r < rule.Rule {
...
func (a *Ab) PeerAdd(c *gin.Context) {
	// forceAlwaysRelay est toujours la chaîne "false"
	//f := &gin.H{}
...
func (a *Ab) PeerUpdate(c *gin.Context) {
	...
	// Supprimer les champs de f s'ils ne sont pas dans allowUp
	for k := range f {
