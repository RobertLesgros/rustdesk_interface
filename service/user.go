...
// InfoByEmail retrieves user information by email
...
// CurUser retrieves the current user from context
...
// ListByGroupId retrieves users by group ID with pagination
...
// ListIdsByGroupId retrieves user IDs by group ID
...
// ListIdAndNameByGroupId retrieves user IDs and names by group ID
...
// CheckUserEnable checks if user is enabled
...
// GetUuidByToken retrieves UUID by user and token
...
// Logout logs out user by deleting token and unbinding UUID
...
// Delete deletes user and associated OAuth information
func (us *UserService) Delete(u *model.User) error {
	...
	// Delete associated address books
...
func (us *UserService) Update(u *model.User) error {
	...
	if us.IsAdmin(currentUser) {
		...
		// Ensure the last admin cannot be disabled or demoted
...
// InfoByOauthId retrieves user information by OAuth name and OpenID
...
// RegisterByOauth registers a user via OAuth
...
// GenerateUsernameByOauth generates a unique username for OAuth users
...
// FindLatestUserIdFromLoginLogByUuid finds last logged-in user by UUID and device ID
...
// IsPasswordEmptyById checks if password is empty by user ID (for OAuth auto-registration)
...
// IsPasswordEmptyByUsername checks if password is empty by username (for OAuth auto-registration)
...
// IsPasswordEmptyByUser checks if password is empty (for OAuth auto-registration)
func (us *UserService) IsPasswordEmptyByUser(u *model.User) bool {
	return us.IsPasswordEmptyById(u.Id)
}

// Register registers a new user, returns nil if username already exists
...
// UserTokenExpireTimestamp generates user token expiration timestamp
...
// IsUsernameExists checks if username exists in local database and LDAP (if enabled)
